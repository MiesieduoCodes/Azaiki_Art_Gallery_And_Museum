...
export default function CollectionForm({ params }: { params: { id: string } }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        const { error } = await supabase.from("collections").update(collectionData).eq("id", params.id)
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center">
          <Link href="/admin/collections" className="mr-4 p-2 rounded-full hover:bg-gray-200">
            <ArrowLeft className="h-5 w-5" />
          </Link>
          <h1 className="text-2xl font-bold text-gray-900">{isEditing ? "Edit Collection" : "Add New Collection"}</h1>
        </div>
